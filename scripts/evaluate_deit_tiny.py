...
def main():
    ...
    checkpoint_path = Path("outputs/checkpoints/deit_tiny_best_model.pt")
    metrics_dir = Path("outputs/metrics")
    metrics_dir.mkdir(parents=True, exist_ok=True)

    metrics_path = metrics_dir / "deit_tiny_test_metrics.json"
    ...
    model = DeiTTinyBinary(img_size=32).to(device)
